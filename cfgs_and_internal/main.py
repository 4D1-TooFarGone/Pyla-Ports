...
def _reset_display_hard(*_a):
    try:
        os.system("/system/bin/wm size reset >/dev/null 2>&1; /system/bin/wm density reset >/dev/null 2>&1")
    except Exception:
        pass

atexit.register(_reset_display_hard)
...
try:
    ...
    _FATAL_SIGS = []
    for _name in ("SIGSEGV", "SIGABRT", "SIGBUS", "SIGFPE", "SIGILL", "SIGTERM"):
        _s = getattr(_signal, _name, None)
        if _s is not None:
            _FATAL_SIGS.append(_s)

    _in_fatal = False

    def _fatal_signal(signum, frame):
        global _in_fatal
        for _s2 in _FATAL_SIGS:
            try:
                _signal.signal(_s2, _signal.SIG_DFL)
            except Exception:
                pass
        if not _in_fatal:
            _in_fatal = True
    ...
    for _s in _FATAL_SIGS:
        try:
            _signal.signal(_s, _fatal_signal)
        except Exception:
            pass
except Exception:
    pass
...
from utils import get_brawler_list, update_missing_brawlers_info, check_version, notify_user, update_wall_model_classes, get_latest_wall_model_file, cprint, is_android
...
def pyla_main(discord_bot, queue_data, stop_event=None, runtime_control=None):
    class Main:
        def __init__(self):
            ...
            model_paths, model_imgsz = self.load_models()
            self.Play = Play(*model_paths, self.window_controller, pyla_code, input_size=(model_imgsz, model_imgsz))
            print("Models loaded.")

            if not self.window_controller.is_brawl_stars_running():
                cprint("Brawl Stars is not running — launching it now...", "#AAE5A4")
        ...
        @staticmethod
        def load_models():
            quality = str(load_toml_as_dict("cfg/general_config.toml").get("model_quality", "stock")).lower()
            tiers = {
                "fast": ("./models/tiers/fast/", 320),
                "standard": ("./models/tiers/standard/", 480),
                "high": ("./models/tiers/high/", 640),
                "stock": ("./models/", 640),
            }
            folder, imgsz = tiers.get(quality, tiers["stock"])
            if not (os.path.exists(folder + "mainInGameModel.onnx") and os.path.exists(folder + "tileDetector.onnx")):
                print(f"Model tier '{quality}' not found at {folder}; falling back to stock 640.")
                folder, imgsz = tiers["stock"]
            print(f"Model quality: {quality} (input {imgsz}px, from {folder})")
            paths = [
            ...
            while True:
                ...
                if abs(s_time - t_now) > 1:
                    ...
                    if elapsed > 0:
                        ...
                        if self.runtime_control:
                            try:
                                current_brawler = self.Stage_manager.brawlers_pick_data[0]['brawler'] if self.Stage_manager.brawlers_pick_data else None
                            except Exception:
                                current_brawler = None
                            self.runtime_control.update_stats(
                                ips=round(ips, 2),
                                state=self.get_latest_state(),
...
if api_base_url != "localhost" and not is_android():
