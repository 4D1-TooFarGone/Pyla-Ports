...
from utils import get_brawler_icon_path, resolve_project_path, is_android
from .runtime import RuntimeManager, running_flag_path
...
class _SuppressQueuePolling(logging.Filter):
    def filter(self, record: logging.LogRecord) -> bool:
        message = record.getMessage()
        return not (
            ('"GET /api/runtime/status ' in message or '"GET /api/stats ' in message)
            and ' 200 -' in message
        )
...
def create_app(pyla_main, start_discord_bot=False):
    ...
    if is_android() and os.path.exists(running_flag_path()):
        def _auto_resume():
            time.sleep(4)
            try:
                result = runtime_manager.start_current_queue(discord_bot)
                print(f"[auto-resume] {result.get('message', result)}", flush=True)
            except Exception as exc:
                print(f"[auto-resume] failed: {exc}", flush=True)
        threading.Thread(target=_auto_resume, daemon=True, name="pyla-auto-resume").start()
    ...
    @app.get("/api/stats")
    def stats():
        return jsonify(runtime_manager.get_live_stats())
    ...
    def benchmark():
        ...
        import time
        import numpy as np
        addr = os.environ.get("PYLA_INFER_ADDR")
        if not addr:
            return jsonify({"ok": False, "message": "GPU inference service not available on this device."})
        try:
            from infer_client import SocketInferBackend
        except Exception as e:
            return jsonify({"ok": False, "message": f"inference client unavailable: {e}"})
        tiers = [("fast", 320), ("standard", 480), ("high", 640)]
        results = []
        for tier, size in tiers:
            try:
                be = SocketInferBackend(addr, f"mainInGameModel@{size}", call_timeout=6.0, cooldown=0.1)
                x = np.zeros((1, 3, size, size), dtype=np.float32)
                ready = False
                for _ in range(150):
                    try:
                        be.infer(x); ready = True; break
                    except Exception:
                        time.sleep(0.4)
                if not ready:
                    results.append({"tier": tier, "error": "load timeout"}); continue
                n = 15
                t0 = time.perf_counter()
                for _ in range(n):
                    be.infer(x)
                ms = (time.perf_counter() - t0) / n * 1000.0
                results.append({"tier": tier, "ms": round(ms, 1),
                                "ips": round(1000.0 / ms, 1), "backend": be.last_backend or "?"})
            except Exception as e:
                results.append({"tier": tier, "error": str(e)[:60]})
        ok = [r for r in results if "ips" in r]
        recommended = None
        for tier in ("high", "standard", "fast"):
            r = next((x for x in ok if x["tier"] == tier), None)
            if r and r["ips"] >= 10:
                recommended = tier; break
        if not recommended and ok:
            recommended = max(ok, key=lambda r: r["ips"])["tier"]
        return jsonify({"ok": True, "results": results, "recommended": recommended})
