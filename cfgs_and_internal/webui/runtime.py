...
def running_flag_path() -> str:
    """Path to the on-disk 'bot is active' flag. It intentionally survives a hard
    process crash (uncatchable native SIGSEGV) — that is what lets Android
    auto-resume after scrcpy/av segfaults during Brawl's launch frame-burst."""
    root = os.environ.get("PYLA_ROOT") or os.getcwd()
    return os.path.join(root, ".pyla_running")


def _write_running_flag() -> None:
    try:
        with open(running_flag_path(), "w") as fh:
            fh.write("1")
    except OSError:
        pass
...
class RuntimeControl:
    def __init__(self, state_callback: Callable[[str], None]):
        ...
        self._stats: dict[str, Any] = {"ips": 0.0, "state": None}
...
class RuntimeManager:
    ...
    def get_live_stats(self) -> dict[str, Any]:
        with self._lock:
            ...
            rc = self.rt_control
            state = self._state
        stats = {"ips": 0.0, "state": None}
        if rc is not None:
            stats.update(rc.get_stats())
        stats["runtime_state"] = state
        stats["is_running"] = thread_alive
        return stats
