...
def is_android():
    """True when running as the on-device Android port (Termux CPython via Shizuku).
    Android's CPython exposes sys.getandroidapilevel; our launcher also sets PYLA_ROOT.
    Used to gate desktop-only behavior (adb-server restart, browser launch, model
    auto-download, GUI debug viewer) that breaks / is pointless on the phone — while
    leaving the desktop Mac/Linux port fully intact."""
    return hasattr(sys, "getandroidapilevel") or bool(os.environ.get("PYLA_ROOT"))
import toml
...
try:
    ...
    def get_player_info(tag):
        tag = str(tag or "").lstrip("#").strip()
        if not tag:
            return None
        try:
            import toml as _toml
            _cfg = _toml.load(str(PROJECT_ROOT / "cfg" / "general_config.toml"))
            api_key = _cfg.get("brawlstars_api_key", "").strip()
        except Exception:
            api_key = ""
        if not api_key:
            return None
    ...
    def get_brawler_stats(player_info, brawler_name, power_level=False):
        if not isinstance(player_info, dict):
            return (None, None, None) if power_level else (None, None)
        name_key = str(brawler_name or "").lower().replace(" ", " ")
        for b in player_info.get("brawlers", []):
            if b.get("name", "").lower() == name_key:
                trophies = b.get("trophies")
                pw = b.get("power")
                return (trophies, 0, pw) if power_level else (trophies, 0)
        return (None, None, None) if power_level else (None, None)
...
class DefaultEasyOCR:
    ...
    def create_reader(self):
        ...
        missing = [f for f in self.REQUIRED_MODELS if not (model_dir / f).exists()]
        download = bool(missing)
        if download:
            print(f"EasyOCR models not found, downloading to {model_dir} (one-time, ~100 MB)...")
        try:
            import easyocr
            try:
                ...
                raise EasyOCRInitializationError(f"EasyOCR failed to load models from {model_dir}: {exc}") from exc
...
def _get_project_root():
    ...
    if getattr(sys, 'frozen', False):
        env_root = os.environ.get('PYLA_ROOT')
        if env_root:
