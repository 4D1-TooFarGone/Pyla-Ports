            ...
            <section class="panel support-panel">
                <div class="support-copy">
                    <div>
                        ...
                        <p class="support-lead">Join the Discord for support, news, and announcements.</p>
                    </div>
                </div>
...
const MODEL_TIERS = [
    { id: "fast",     label: "Fast",     sub: "320 px",   acc: 72, spd: 100,
      desc: "Fastest, lowest accuracy — can miss small/distant enemies. Best for weak phones." },
    { id: "standard", label: "Standard", sub: "480 px",   acc: 85, spd: 70,
      desc: "Balanced — the everyday choice. Reads the board well without overworking the phone." },
    { id: "high",     label: "High",     sub: "640 px",   acc: 94, spd: 48,
      desc: "Sharp detection for higher trophies. Best paired with GPU acceleration." },
    { id: "stock",    label: "Stock",    sub: "original", acc: 97, spd: 44,
      desc: "The original Pyla models, untouched — most accurate, identical to PC." },
];

function renderModelsPanel() {
    const g = (state.bootstrap.settings && state.bootstrap.settings.general) || {};
    const tier = g.model_quality || "standard";
    const accelOn = (g.cpu_or_gpu || "auto") !== "cpu";
    const fps = String(g.limit_game_fps || "off").toLowerCase();
    const dscale = String(g.game_downscale || "off").toLowerCase();
    const t = MODEL_TIERS.find((x) => x.id === tier) || MODEL_TIERS[1];
    const fpsSeg = ["off", "30", "60"].map((f) => `
        <button class="mq-fps ${f === fps ? "is-active" : ""}" data-fps="${f}">${f === "off" ? "Off" : f}</button>`).join("");
    const dsSeg = [["off", "Off"], ["0.7", "70%"], ["0.5", "50%"]].map(([v, l]) => `
        <button class="mq-ds ${v === dscale ? "is-active" : ""}" data-ds="${v}">${l}</button>`).join("");
    const segs = MODEL_TIERS.map((x) => `
        <button class="mq-seg ${x.id === tier ? "is-active" : ""}" data-tier="${x.id}">
    ...
    return `
        <section class="panel models-panel">
            ...
            <div class="mq-seg-row">${segs}</div>
            <div class="mq-detail">
                <p class="desc" id="mqDesc">${escapeHtml(t.desc)}</p>
                <div class="mq-bars">
                    <div class="mq-bar"><span>Accuracy</span><div class="mq-track"><div class="mq-fill acc" style="width:${t.acc}%"></div></div></div>
                    <div class="mq-bar"><span>Speed</span><div class="mq-track"><div class="mq-fill spd" style="width:${t.spd}%"></div></div></div>
                </div>
            </div>
            <div class="mq-toggle-row">
                <div class="mq-toggle-txt">
                    <b>Hardware acceleration</b>
                    <p class="meta">Use the phone's GPU/NPU when available (falls back to CPU automatically).</p>
                </div>
                <button class="mq-toggle ${accelOn ? "is-on" : ""}" id="mqAccel" role="switch" aria-pressed="${accelOn}" aria-label="Hardware acceleration"></button>
            </div>
            <div class="mq-toggle-row">
                <div class="mq-toggle-txt">
                    <b>Limit game FPS <span class="mq-hint-tag">helps IPS</span></b>
                    <p class="meta">Cap Brawl's frame rate so it stops hogging the GPU — leaves more for detection.</p>
                </div>
                <div class="mq-fps-seg">${fpsSeg}</div>
            </div>
            <div class="mq-toggle-row">
                <div class="mq-toggle-txt">
                    <b>Brawl render scale <span class="mq-hint-tag">frees GPU</span></b>
                    <p class="meta">Render Brawl at a fraction of resolution (the bot still captures full 1080p). Less GPU work for the game = more for detection.</p>
                </div>
                <div class="mq-fps-seg">${dsSeg}</div>
            </div>
            <button class="mq-tune" id="mqTune">⚙ Auto-Tune for this phone</button>
            <div class="mq-results" id="mqResults"></div>
            <p class="meta mq-note">Changes apply on the next <b>Start</b> (models load when the bot starts).</p>
        </section>
    `;
}

function bindModelsPanel() {
    document.querySelectorAll(".mq-seg").forEach((btn) => {
        btn.addEventListener("click", async () => {
            const tier = btn.dataset.tier;
            await saveGeneralSetting({ model_quality: tier });
            renderDashboard();
            showToast(`Model quality set to ${tier} — applies on next Start.`, "success");
        });
    });
    const accel = document.getElementById("mqAccel");
    accel?.addEventListener("click", async () => {
        const on = accel.getAttribute("aria-pressed") !== "true";
        await saveGeneralSetting({ cpu_or_gpu: on ? "auto" : "cpu" });
        renderDashboard();
    });
    document.querySelectorAll(".mq-fps").forEach((btn) => {
        btn.addEventListener("click", async () => {
            const f = btn.dataset.fps;
            await saveGeneralSetting({ limit_game_fps: f });
            renderDashboard();
            showToast(f === "off" ? "Game FPS uncapped." : `Brawl capped to ${f} fps — applies on next Start.`, "success");
        });
    });
    document.querySelectorAll(".mq-ds").forEach((btn) => {
        btn.addEventListener("click", async () => {
            const d = btn.dataset.ds;
            await saveGeneralSetting({ game_downscale: d });
            renderDashboard();
            showToast(d === "off" ? "Brawl render scale: full." : `Brawl render scale ${Math.round(parseFloat(d) * 100)}% — applies on next Start.`, "success");
        });
    });
    document.getElementById("mqTune")?.addEventListener("click", runAutoTune);
}

async function runAutoTune() {
    const btn = document.getElementById("mqTune");
    const box = document.getElementById("mqResults");
    if (!btn || !box) return;
    btn.disabled = true; btn.textContent = "⏳ Benchmarking each tier on this phone…";
    box.innerHTML = "";
    try {
        const res = await fetchJSON("/api/benchmark", { method: "POST" }, true);
        if (!res || res.ok === false) {
            box.innerHTML = `<p class="meta">${escapeHtml(res?.message || "Benchmark unavailable — start the bot first so the GPU service is up.")}</p>`;
            return;
        }
        const rows = (res.results || []).map((r) => r.error
            ? `<tr><td>${escapeHtml(r.tier)}</td><td colspan="2" class="meta">${escapeHtml(r.error)}</td></tr>`
            : `<tr class="${r.tier === res.recommended ? "mq-win" : ""}"><td>${escapeHtml(r.tier)}</td><td>${escapeHtml(r.backend || "?")}</td><td class="mq-n">${r.ips} IPS</td></tr>`).join("");
        box.innerHTML = `
            <table class="mq-table"><thead><tr><th>Tier</th><th>Backend</th><th class="mq-n">Speed</th></tr></thead><tbody>${rows}</tbody></table>
            ${res.recommended ? `<p class="meta">Recommended for this phone: <b>${escapeHtml(res.recommended)}</b>. <a href="#" id="mqApply">Apply</a></p>` : ""}`;
        document.getElementById("mqApply")?.addEventListener("click", async (e) => {
            e.preventDefault();
            await saveGeneralSetting({ model_quality: res.recommended });
            renderDashboard();
            showToast(`Applied ${res.recommended} — starts on next Start.`, "success");
        });
    } catch (err) {
        box.innerHTML = `<p class="meta">Benchmark failed: ${escapeHtml(err.message || "error")}</p>`;
    } finally {
        btn.disabled = false; btn.textContent = "⚙ Re-run Auto-Tune";
    }
}

async function saveGeneralSetting(patch) {
    try {
...
function renderSelectedBrawlerEditor(brawler) {
    ...
    const currentTrophies = existing?.trophies ?? 0;
        ...
        <div class="queue-editor">
            <div class="selected-brawler-top">
                ...
                <div>
                    ...
                    <p class="meta-line">Configure the brawler's current trophies and target.</p>
                </div>
            </div>
    ...
    return `
        <div class="setting-row">
            ...
            <div class="setting-input-wrap ${field.suffix ? "has-suffix" : ""}">
                <input data-setting-section="${section}" data-setting-key="${field.key}" type="${field.type}" step="${field.step || "1"}" placeholder="${escapeHtml(field.placeholder || "")}" value="${escapeHtml(formatSettingValue(field, value))}">
...
function bindRuntimeButtons() {
    ...
    document.getElementById("resumeRuntimeBtn")?.addEventListener("click", async () => {
        const result = await fetchJSON("/api/runtime/start", { method: "POST" }, true);
        if (!result.ok) {
            showToast(result.message || "Unable to resume Pyla.", "error");
            return;
        }
...
async function saveQueueItem() {
    ...
    const payload = {
        ...
        trophies: Number(document.getElementById("queueTrophies")?.value || existing?.trophies || 0),
        wins: Number(document.getElementById("queueWins")?.value || existing?.wins || 0),
        win_streak: Number(document.getElementById("queueWinStreak")?.value || existing?.win_streak || 0),
